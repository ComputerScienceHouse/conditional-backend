...
#[post("/forms/intro")]
#[put("/forms/intro")]
pub async fn submit_intro_form(state: Data<AppState>, body: IntroFormSubmission) -> impl Responder {
    match query!("UPDATE freshman_eval_data SET social_events = $1, other_notes = $2 WHERE uid = $3", body.social_events, body.comments, body.uid)
        .execute(&state.db)
        .await
    {
        Ok(_) => HttpResponse::Ok(),
...
pub async fn get_intro_form(state: Data<AppState>) -> impl Responder {
    ...
    match query_as!(FreshmanEvaluation, "SELECT * FROM freshman_eval_data WHERE eval_date > $1 uid = $2", user, &state.year_start)
        .fetch_all(&state.db)
        .await
    {
...
#[get("/forms/intro")]
pub async fn get_intro_form(state: Data<AppState>) -> impl Responder {
    match query_as!(FreshmanEvaluation, "SELECT * FROM freshman_eval_data WHERE eval_date > $1", &state.year_start)
        .fetch_all(&state.db)
        .await
    {
