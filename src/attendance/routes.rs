...
pub async fn submit_seminar_attendance(
    ...
    let usernames: Vec<String> = body.body;
    match split_usernames(usernames) {
        Ok((frosh, members)) => {
...
pub async fn get_seminars(state: Data<AppState>) -> impl Responder {
    ...
    match query_as!(
        Seminar,
        "SELECT * FROM technical_seminars WHERE timestamp > ($1::timestamp)",
        &state.year_start
    )
    .fetch_all(&state.db)
    .await
...
pub async fn submit_committee_attendance(
    state: Data<AppState>,
    body: SeminarAttendance,
) -> impl Responder {
    // TODO: eboard should auto approve
...
pub async fn get_committee(state: Data<AppState>) -> impl Responder {
    ...
    match query_as!(
        Committee,
        "SELECT * FROM committee_meetings WHERE timestamp > ($1::timestamp)",
        &state.year_start
    )
    .fetch_all(&state.db)
    .await
